...
/// 宿主句柄 (C ABI 层持有上下文与管理器; 由 host_create/host_destroy 管理生命周期)
struct HostHandle {
    HostContext                         ctx;
...
auto onHostThread(MusicxxHostManager* mgr, Fn&& fn) -> decltype(fn()) {
    return pluginxx::ioCallSync<decltype(fn())>(
        mgr,
        std::function<decltype(fn())()>(std::forward<Fn>(fn))
    );
}
...
MUSICXX_EXTERN_PLUGIN_EXPORT int32_t MUSICXX_EXTERN_PLUGIN_CALL
    ...
    setOut(
        out,
        std::string_view{src->data ? src->data : "", static_cast<size_t>(src->size)}
    );
...
MUSICXX_EXTERN_PLUGIN_EXPORT int32_t MUSICXX_EXTERN_PLUGIN_CALL
    musicxx_extern_plugin_host_start(MusicxxExternPluginHost* h, MusicxxExternPluginString* log) {
...
MUSICXX_EXTERN_PLUGIN_EXPORT int32_t MUSICXX_EXTERN_PLUGIN_CALL
    ...
    return pluginxx::guardCall(
        ...
        [&]() -> int32_t {
            std::string err;
            const auto  rc = handle->manager->stop(timeout_ms, err);
            if (rc != MUSICXX_EXTERN_PLUGIN_OK) {
                setErr(log, err);
            }
            return rc;
...
MUSICXX_EXTERN_PLUGIN_EXPORT int32_t MUSICXX_EXTERN_PLUGIN_CALL
    ...
    std::string err;
    const auto  rc = handle->manager->loadPlugin(
        viewToStd(id_or_path),
...
MUSICXX_EXTERN_PLUGIN_EXPORT int32_t MUSICXX_EXTERN_PLUGIN_CALL
    ...
    std::string       err;
    if (handle->manager->unloadPlugin(pluginId, 5000, err) != MUSICXX_EXTERN_PLUGIN_OK) {
        setErr(log, err);
        return MUSICXX_EXTERN_PLUGIN_ERR_STATE;
    }
    return handle->manager->loadPlugin(pluginId, "{}", true, 10000, err) == MUSICXX_EXTERN_PLUGIN_OK
...
MUSICXX_EXTERN_PLUGIN_EXPORT int32_t MUSICXX_EXTERN_PLUGIN_CALL
    ...
    std::string err;
    const auto  rc = onHostThread(handle->manager.get(), [&]() -> int32_t {
        return handle->manager->setPluginArgs(viewToStd(id), viewToStd(args_json), err);
    });
    if (rc != MUSICXX_EXTERN_PLUGIN_OK) {
        setErr(log, err);
    }
...
MUSICXX_EXTERN_PLUGIN_EXPORT int32_t MUSICXX_EXTERN_PLUGIN_CALL
    ...
    std::string err;
    const auto  rc = handle->manager->stateUpdate(viewToStd(key), viewToStd(value_json), err);
    if (!err.empty()) {
        setErr(log, err);
    }
    return rc;
