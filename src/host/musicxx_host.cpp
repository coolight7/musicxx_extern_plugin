...
public:
  ...
  ~StderrLogSink() override { shutdownThread(); }

protected:
  void onLog(const utilxx_base::LogEntry &entry) override {
    std::fprintf(stderr, "[musicxx_ext][%s] %s\n", levelName(entry.level),
                 entry.message.c_str());
    std::fflush(stderr);
  }
...
///
/// - 注册方必须持有 shared_ptr (分发器只持 weak_ptr),
/// 因此把强引用放在**故意泄漏**的
...
void ensureStderrLogSink() {
  ...
  if (!env || *env == '\0' || std::string_view{env} == "0") {
    holder =
        new std::shared_ptr<StderrLogSink>{}; ///< 记住"已检查过", 不重复判断
    return;
  }
  holder =
      new std::shared_ptr<StderrLogSink>{std::make_shared<StderrLogSink>()};
...
public:
  size_t subscribe(std::string_view topic,
    ...
    subscribers_.push_back(
        Subscriber{std::string{topic}, id, std::move(handler)});
    return id;
  }
...
HostContext::HostContext() : guard(asio::make_work_guard(io)) {}
...
std::shared_ptr<MusicxxHostManager>
MusicxxHostManager::create(HostContext &ctx,
  ...
  ensureStderrLogSink();
  auto mgr = std::shared_ptr<MusicxxHostManager>(
      new MusicxxHostManager(ctx.io.get_executor()));
  mgr->ctx_ = &ctx;
...
void MusicxxHostManager::applyConfig(const MusicxxExternPluginHostConfig &cfg) {
  auto setIfPresent = [](std::string &dst,
                         const MusicxxExternPluginStringView &v) {
...
int32_t MusicxxHostManager::start(std::string &err) {
  ...
  // 工作线程池 (插件 offload 用) + 宿主线程 (唯一动态库插件执行线程)
  if (!ctx_->workers) {
  ...
  // PluginManagerBase 记录的是构造线程; 若不在宿主线程上重绑, 各 vtable
  // 入口会误判 "当前不在 IO 线程" → 投递回宿主线程并同步等待,
  // 而宿主线程正卡在该调用栈里 (插件 start/create/处理器都在这条线程上跑) →
  // 单线程执行器自锁。
  if (!bindIoThread()) {
  ...
  XX_LOGI("[musicxx_ext] host started (platform={}, safeMode={}, noNative={}, "
          "noJs={}, jsRuntime={})",
          platform_, (flags_ & MUSICXX_EXTERN_PLUGIN_FLAG_SAFE_MODE) != 0,
...
bool MusicxxHostManager::bindIoThread() {
  ...
  asio::post(ex, [self, slot, ex] {
    ...
    XX_LOGI("[musicxx_ext] 宿主 IO 线程已绑定 (thread={}, "
            "动态库插件代码只在该线程执行)",
            currentThreadIdText());
    slot->set(true);
  });
...
int32_t MusicxxHostManager::stop(uint32_t timeoutMs, std::string &err) {
  ...
  const auto budget = std::chrono::milliseconds{
      static_cast<int64_t>((std::max)(timeoutMs, 1000u))};
  asio::co_spawn(
      ctx_->io,
  ...
  // v1: 宿主为一次性对象 (start 之后 stop 不再重启); 需要重新启动时由 Dart
  // 侧销毁宿主并重建
...
void MusicxxHostManager::pushEvent(const std::string &type,
  ...
  void *wakeUd = nullptr;
  {
    ...
    ev["plugin"] = plugin;
    Json payload =
        payloadJson.empty() ? Json::object() : parseJsonSafe(payloadJson);
    ev["payload"] = payload;
...
void MusicxxHostManager::pushHostError(const std::string &code,
...
void MusicxxHostManager::setInternalActionRelay(
...
int32_t MusicxxHostManager::pushActionRequestEvent(
    const std::string &pluginId, int64_t requestId, const std::string &action,
...
void MusicxxHostManager::cancelActionsOfInstance(
...
int32_t
MusicxxHostManager::publishPluginEventFrom(const std::string &pluginId,
  ...
  asio::post(ctx_->io, [self, topic, payload] {
    self->publishOnHostThread(topic, payload);
  });
  return MUSICXX_EXTERN_PLUGIN_OK;
}
...
std::string MusicxxHostManager::language() { return language_; }
...
std::string
MusicxxHostManager::pluginInstanceJson(const MusicxxHostInstance &inst) const {
  Json j;
...
MusicxxHostManager::selfRef() {
  return shared_from_this();
}

std::shared_ptr<MusicxxHostInstance>
MusicxxHostManager::createInstance(std::string name) {
...
MusicxxHostManager::resolveInstance(const std::string &idOrInstance) const {
  ...
  // 先按实例名找 (动态库插件: 实例名 = 插件 id; JS 插件: "js:<id>")
...
void MusicxxHostManager::clearPluginRegistrations(
    const std::string &instanceName) {
  for (auto it = hooks_.begin(); it != hooks_.end();) {
    auto &handlers = it->second;
    handlers.erase(std::remove_if(handlers.begin(), handlers.end(),
                                  [&](const HookHandler &h) {
                                    return h.plugin == instanceName;
                                  }),
                   handlers.end());
    if (handlers.empty()) {
      it = hooks_.erase(it);
...
int32_t MusicxxHostManager::scanPlugins(std::string &outJson,
                                        std::string &err) {
  if (!running_.load(std::memory_order_acquire)) {
...
int32_t MusicxxHostManager::loadPlugin(const std::string &idOrPath,
  ...
  std::string pluginId;
  {
    ...
    if (fs::is_directory(p, ec)) {
      ...
      std::vector<std::string> depends;
      std::vector<std::string> optionalDepends;
      if (pluginxx::parsePluginManifest(p, manifestName, manifestEntry, depends,
                                        optionalDepends, nullptr, nullptr) &&
          !manifestName.empty()) {
       ...
       created]() -> asio::awaitable<void> {
        ...
        if (manifestKind == "js") {
          ...
          if (!jsInst) {
            ...
            *failMsg = "JS 插件装载失败 (脚本错误或 start 事务失败; "
                       "详见日志与调试信息)";
            Json payload;
            payload["id"] = pluginId;
            payload["phase"] = "load";
        ...
        if (options->args.contains("kind") &&
            options->args["kind"].is_string()) {
...
int32_t MusicxxHostManager::enablePlugin(const std::string &id,
                                         std::string &err) {
  if (!running_.load(std::memory_order_acquire)) {
...
int32_t MusicxxHostManager::disablePlugin(const std::string &id,
                                          std::string &err) {
  if (!running_.load(std::memory_order_acquire)) {
...
int32_t MusicxxHostManager::unloadPlugin(const std::string &id,
                                         uint32_t timeoutMs, std::string &err) {
  if (!running_.load(std::memory_order_acquire)) {
    err = "plugin_unload: host not started";
    return MUSICXX_EXTERN_PLUGIN_ERR_STATE;
  }
  // 幂等: 未加载视为"已经是未加载状态"; 插件 id 与实例名都接受 (JS 插件实例名为
  // js:<id>)
  auto inst = resolveInstance(id);
  if (!inst) {
...
int32_t MusicxxHostManager::setPluginArgs(const std::string &id,
...
std::string
MusicxxHostManager::deriveConfigPath(const std::string &pluginId,
...
int32_t MusicxxHostManager::pluginConfigPath(const std::string &id,
                                             std::string &out,
                                             std::string &err) {
  auto inst = resolveInstance(id);
...
void PLUGINXX_CALL pluginCallDone(void *ud, int32_t status,
                                  const PluginxxStringView *payload) noexcept {
...
void PLUGINXX_CALL capabilityCallTrampoline(
  ...
  std::unique_ptr<CapabilityCallBridge> holder{
      static_cast<CapabilityCallBridge *>(ud)};
  if (!holder) {
    return;
  }
  std::string text;
  ...
  if (holder->done) {
    holder->done(status == PLUGINXX_OPERATOR_OK
...
void MusicxxHostManager::invokeCapabilityOnHostThread(
  ...
  if (!entry || !entry->start) {
    fail(MUSICXX_EXTERN_PLUGIN_ERR_NOT_FOUND,
         "插件未声明该能力 (需要在 start 事务里注册): " + cap);
    return;
  }
  if (entry->provider != inst->name) {
    fail(MUSICXX_EXTERN_PLUGIN_ERR_PERMISSION,
  ...
  auto *handle = invokeCapabilityAsync(
      ...
      argsView, &capabilityCallTrampoline, bridge, &callErr);
  if (!handle) {
...
int32_t MusicxxHostManager::pluginCall(const std::string &id,
  ...
  // 第一步在宿主线程执行: 实例查找、能力归属校验、发起异步调用
  // (完成回调也在该线程)
  asio::post(ctx_->io, [self, state, id, method, argsJson] {
    self->invokeCapabilityOnHostThread(
        id, method, argsJson, [state](int32_t rc, const std::string &text) {
          if (rc == MUSICXX_EXTERN_PLUGIN_OK) {
            state->payload = text;
          } else {
            state->error =
                text.empty() ? std::string{"plugin_call failed"} : text;
          }
          state->slot.set(rc);
        });
  });
...
void MusicxxHostManager::pluginCallAsync(
  ...
  auto self = shared_from_this();
  asio::post(ctx_->io, [self, id, method, argsJson,
...
std::string MusicxxHostManager::qualifyEventTopic(std::string_view topic) {
  ...
  XX_LOGW("[musicxx_ext] 拒绝非法事件主题 `{}` (必须是 musicxx.* 或 "
          "plugin.<pluginId>.*)",
          topic);
  return {};
}

int32_t MusicxxHostManager::publishPluginEvent(const std::string &topic,
                                               const std::string &payloadJson) {
  if (!running_.load(std::memory_order_acquire)) {
    return MUSICXX_EXTERN_PLUGIN_ERR_STATE;
  }
  ...
  if (topic.rfind("musicxx.", 0) != 0) {
    XX_LOGW("[musicxx_ext] 宿主发布事件被拒绝: 主题 `{}` 必须以 musicxx. 开头",
            topic);
    return MUSICXX_EXTERN_PLUGIN_ERR_PERMISSION;
  }
  const std::string payload =
...
int32_t
MusicxxHostManager::publishOnHostThread(const std::string &topic,
                                        const std::string &payloadJson) {
...
/* ==================== 状态镜像 (Dart → 原生) ==================== */
int32_t MusicxxHostManager::stateUpdate(const std::string &key,
...
int32_t MusicxxHostManager::stateUpdateBatch(const std::string &itemsJson,
                                             std::string &err) {
  bool parseOk = false;
  ...
  for (const auto &item : items) {
    ...
    std::string subErr;
    const auto sub = stateUpdate(item["key"].get<std::string>(),
                                 item["value"].dump(), subErr);
...
int32_t MusicxxHostManager::debugInfo(std::string &outJson) {
  ...
  {
    std::lock_guard<std::mutex> lock{eventMutex_};
    j["queue"] = {{"size", static_cast<int64_t>(events_.size())},
                  {"dropped", eventDropped_}};
  }
...
int32_t MusicxxHostManager::statsJson(const std::string * /*scopeJson*/,
                                      std::string &outJson) {
  Json j;
  Json host;
  host["uptimeMs"] =
      running_.load() ? std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  if (statsEnabled_) {
    {
      std::lock_guard<std::mutex> lock{eventMutex_};
      host["queue"] = {{"size", static_cast<int64_t>(events_.size())},
                       {"dropped", eventDropped_}};
    }
  ...
  for (const auto &[name, inst] : loaded_) {
    ...
    p["kind"] = inst->kind;
    p["phaseMs"] = {
        {"load", inst->phaseMs.load},       {"create", inst->phaseMs.create},
        {"start", inst->phaseMs.start},     {"stop", inst->phaseMs.stop},
    ...
    const int64_t uptimeMs =
        inst->loadedAtMs > 0 ? (steadyNowMs() - inst->loadedAtMs) : 0;
...
int32_t MusicxxHostManager::statsConfig(const std::string &cfgJson,
                                        std::string &err) {
  bool parseOk = false;
...
int32_t MusicxxHostManager::setConfig(const std::string &cfgJson,
                                      std::string &err) {
  bool parseOk = false;
  Json cfg = parseJsonSafe(cfgJson, &parseOk);
  if (!parseOk || !cfg.is_object()) {
