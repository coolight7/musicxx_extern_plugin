/// musicxx 外部插件原生宿主: 声明式 UI 扩展 (plan §5.6)
///
/// 插件不写 Flutter 代码, 只做**声明**: 注册一个 UI 项 (入口项 / 菜单项 / 设置页 /
/// 附加信息块) 并给出类型相关的 JSON 内容, 由 Dart 侧渲染 (声明式描述 + 宿主渲染)。
///
/// 设计要点:
/// - **注册表归宿主线程独占** (plan §2.3.2 的无锁不变式): 注册/更新/摘除/快照都在宿主线程;
///   Dart 线程的 `ui_snapshot` 经 C ABI 侧的 `onHostThread` 投递后读取;
/// - **命名空间校验**: 项 id 必须是本插件命名空间 (`plugin.<pluginId>.<名>`), 短名由宿主补前缀;
///   动作里的 `route` 只允许指向本插件页面, `action` 只允许官方 `musicxx.*` 动作;
/// - **结构校验在注册时做**: data 必须是 JSON 对象且按类型含必需字段 (title 等),
///   早报错好过让 Dart 侧渲染时才发现;
/// - **生命周期**: 项随插件实例 stop/卸载自动摘除 (宿主在 `clearPluginRegistrations` 里兜底),
///   每次变化推送 `musicxx.ui.changed` 事件 (payload 带该插件的全部项, Dart 整批替换);
/// - **只存声明**: 宿主不解释 data 的业务含义 (除了动作描述的结构), 渲染细节留给 Dart 侧。
#include "host_json.h"
...
/// 单条 UI 项 data 的体积上限 (与 plan §4.7 的状态镜像限制同口径)
...
bool requiresTitle(std::string_view type) {
    return type == MUSICXX_PLUGIN_UI_TYPE_HOME_ENTRY || type == MUSICXX_PLUGIN_UI_TYPE_SONG_ACTION
...
/// - `{"kind":"none"}`
bool validateAction(const Json& action, std::string_view pluginId, std::string& err) {
    if (action.is_null()) {
    ...
    const std::string kind
        = (action.contains("kind") && action["kind"].is_string()) ? action["kind"].get<std::string>() : "none";
    ...
    if (kind == MUSICXX_PLUGIN_UI_ACTION_HOST) {
        ...
        // 官方动作只能调 `musicxx.*`; 插件自定义动作 (`plugin.<自己id>.*`) 不作为 UI 动作使用
...
/// 由 Dart 侧调用时按能力表判定)
std::string normalizeItemId(std::string_view raw, std::string_view pluginId, bool& ok) {
...
int32_t MusicxxHostManager::registerUiEntry(
    ...
    if (spec.struct_size != 0 && spec.struct_size < sizeof(MusicxxPluginUIEntrySpec)) {
        return MUSICXX_EXTERN_PLUGIN_ERR_ARG;
    }
    if (spec.flags != 0) {
    ...
    if (inst->unloadRequested || !inst->enabled) {
        XX_LOGW("[musicxx_ext] 插件 `{}` 注册 UI 项被拒绝: 实例正在关闭或已禁用", inst->name);
        return MUSICXX_EXTERN_PLUGIN_ERR_STATE;
    }
    if (!spec.item_id.data || spec.item_id.size == 0 || !spec.type.data || spec.type.size == 0) {
        return MUSICXX_EXTERN_PLUGIN_ERR_ARG;
    }
    ...
    if (!isKnownUiType(type)) {
        XX_LOGW("[musicxx_ext] 插件 `{}` 注册未知 UI 项类型 `{}` 被拒绝", inst->name, type);
        return MUSICXX_EXTERN_PLUGIN_ERR_NOT_FOUND;
    }

    bool       idOk = false;
    const std::string itemId = normalizeItemId(
        std::string_view{spec.item_id.data, static_cast<size_t>(spec.item_id.size)},
        pluginId,
        idOk
    );
    if (!idOk) {
        XX_LOGW("[musicxx_ext] 插件 `{}` 注册 UI 项 `{}` 被拒绝: 命名空间非法", inst->name, itemId);
        return MUSICXX_EXTERN_PLUGIN_ERR_PERMISSION;
    }

    const std::string dataJson
        = spec.data_json.data
              ? std::string{spec.data_json.data, static_cast<size_t>(spec.data_json.size)}
              : std::string{"{}"};
    std::string err;
    if (!validateUiData(type, dataJson, pluginId, err)) {
        XX_LOGW("[musicxx_ext] 插件 `{}` 注册 UI 项 `{}` 被拒绝: {}", inst->name, itemId, err);
        return MUSICXX_EXTERN_PLUGIN_ERR_ARG;
    }
    ...
    if (existing == uiEntries_.end()) {
        ...
        if (owned >= kMaxUiEntriesPerPlugin) {
            XX_LOGW(
                ...
                inst->name,
                kMaxUiEntriesPerPlugin,
                itemId
            );
            return MUSICXX_EXTERN_PLUGIN_ERR_QUEUE_FULL;
        }
    } else if (existing->second.pluginId != pluginId) {
        // 理论上 normalizeItemId 已经挡掉; 这里再兜一层 (防止 id 规范化逻辑变动引入漏洞)
        XX_LOGW("[musicxx_ext] 插件 `{}` 试图覆盖他人 UI 项 `{}`", inst->name, itemId);
        return MUSICXX_EXTERN_PLUGIN_ERR_PERMISSION;
    }
...
int32_t MusicxxHostManager::unregisterUiEntry(MusicxxHostInstance* inst, std::string_view itemId) {
    if (itemId.empty()) {
        return MUSICXX_EXTERN_PLUGIN_ERR_ARG;
    }
    ...
    if (inst && it->second.pluginId != pluginId) {
        XX_LOGW("[musicxx_ext] 插件 `{}` 试图注销他人 UI 项 `{}`", inst->name, itemId);
        return MUSICXX_EXTERN_PLUGIN_ERR_PERMISSION;
    }
...
int32_t MusicxxHostManager::updateUiEntry(
    MusicxxHostInstance* inst,
...
int32_t MusicxxHostManager::listUiEntries(MusicxxHostInstance* inst, std::string& outJson) {
    const std::string pluginId = inst ? pluginIdOf(inst->name) : std::string{};
    outJson                    = uiItemsJson(pluginId);
    return MUSICXX_EXTERN_PLUGIN_OK;
}

int32_t MusicxxHostManager::notifyUi(MusicxxHostInstance* inst, std::string_view messageJson) {
    if (!inst) {
        return MUSICXX_EXTERN_PLUGIN_ERR_ARG;
    }
...
int32_t PLUGINXX_CALL xx_ui_unregister_entry(
    const PluginxxHost*       host,
    ...
    return pluginxx::guardVtableCall(MUSICXX_EXTERN_PLUGIN_ERR_INTERNAL, [&]() -> int32_t {
        ...
        auto                    mgr = call.manager();
        auto                    inst = call.instance();
        const std::string_view  id{itemId->data, static_cast<size_t>(itemId->size)};
        return pluginxx::ioCallSyncKeep<int32_t>(call, mgr, [mgr, inst, id]() -> int32_t {
            return mgr->unregisterUiEntry(inst, id);
        });
    });
}

int32_t PLUGINXX_CALL xx_ui_update_entry(
    const PluginxxHost*       host,
    const PluginxxStringView* itemId,
    ...
    return pluginxx::guardVtableCall(MUSICXX_EXTERN_PLUGIN_ERR_INTERNAL, [&]() -> int32_t {
        auto call = pluginxx::enterPluginHost<MusicxxHostInstance, MusicxxHostManager>(host);
        if (!call.ok() || !itemId || !itemId->data || !dataJson || !dataJson->data) {
            return MUSICXX_EXTERN_PLUGIN_ERR_ARG;
        }
        auto                    mgr = call.manager();
        auto                    inst = call.instance();
        const std::string_view  id{itemId->data, static_cast<size_t>(itemId->size)};
        const std::string_view  data{dataJson->data, static_cast<size_t>(dataJson->size)};
        return pluginxx::ioCallSyncKeep<int32_t>(call, mgr, [mgr, inst, id, data]() -> int32_t {
            return mgr->updateUiEntry(inst, id, data);
        });
    });
}

int32_t PLUGINXX_CALL xx_ui_list_entries(const PluginxxHost* host, PluginxxString* outJson) {
    return pluginxx::guardVtableCall(MUSICXX_EXTERN_PLUGIN_ERR_INTERNAL, [&]() -> int32_t {
        if (!outJson) {
            return MUSICXX_EXTERN_PLUGIN_ERR_ARG;
        }
        ...
        auto        inst = call.instance();
        std::string json;
        const int32_t rc = pluginxx::ioCallSyncKeep<int32_t>(call, mgr, [mgr, inst, &json]() -> int32_t {
...
int32_t PLUGINXX_CALL xx_ui_notify(const PluginxxHost* host, const PluginxxStringView* messageJson) {
    return pluginxx::guardVtableCall(MUSICXX_EXTERN_PLUGIN_ERR_INTERNAL, [&]() -> int32_t {
        auto call = pluginxx::enterPluginHost<MusicxxHostInstance, MusicxxHostManager>(host);
        ...
        auto                    mgr = call.manager();
        auto                    inst = call.instance();
        const std::string_view  message{messageJson->data, static_cast<size_t>(messageJson->size)};
        return pluginxx::ioCallSyncKeep<int32_t>(call, mgr, [mgr, inst, message]() -> int32_t {
